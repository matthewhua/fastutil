...
public class FloatBigArrayBigList extends AbstractFloatBigList implements RandomAccess, Cloneable, java.io.Serializable {
	...
	private class SubList extends AbstractFloatBigList.FloatRandomAccessSubList {
	 ...
	 private final class SubListIterator extends FloatBigListIterators.AbstractIndexBasedBigListIterator {
	  ...
	  @Override
	  protected final float get(long i) { return BigArrays.get(a, from + i); }
	  @Override
	  protected final void add(long i, float k) { SubList.this.add(i, k); }
	  @Override
	  protected final void set(long i, float k) { SubList.this.set(i, k); }
	  @Override
	  protected final void remove(long i) { SubList.this.removeFloat(i); }
	  @Override
	  protected final long getMaxPos() { return to - from; }
	  @Override
	  public float nextFloat() { if (! hasNext()) throw new NoSuchElementException(); return BigArrays.get(a, from + (lastReturned = pos++)); }
	  @Override
	  public float previousFloat() { if (! hasPrevious()) throw new NoSuchElementException(); return BigArrays.get(a, from + (lastReturned = --pos)); }
	  @Override
	 ...
	 private final class SubListSpliterator extends FloatBigSpliterators.LateBindingSizeIndexBasedSpliterator {
	  ...
	  protected final long getMaxPosFromBackingStore() { return to; }
	   @Override
	  protected final float get(long i) { return BigArrays.get(a, i); }
	  @Override
	...
	public FloatBigListIterator listIterator(final long index) {
	 ...
	 return new FloatBigListIterator () {
	   ...
	   @Override
	   public float nextFloat() { if (! hasNext()) throw new NoSuchElementException(); return BigArrays.get(a, last = pos++); }
	   @Override
	   public float previousFloat() { if (! hasPrevious()) throw new NoSuchElementException(); return BigArrays.get(a, last = --pos); }
	   @Override
