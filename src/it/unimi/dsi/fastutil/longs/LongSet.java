...
public interface LongSet extends LongCollection , Set<Long> {
	...
	default LongSpliterator spliterator() {
	 return LongSpliterators.asSpliterator(
	   iterator(), sizeOf(this), LongSpliterators.SET_SPLITERATOR_CHARACTERISTICS);
	}
	...
	public static LongSet of(long... a) {
	 ...
	 LongSet innerSet = a.length <= LongSets.ARRAY_SET_CUTOFF ?
	  new LongArraySet (a.length) :
	  new LongOpenHashSet (a.length);
	 for (long element : a) {
