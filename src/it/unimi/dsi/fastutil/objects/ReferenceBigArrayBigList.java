...
public class ReferenceBigArrayBigList <K> extends AbstractReferenceBigList <K> implements RandomAccess, Cloneable, java.io.Serializable {
	...
	private static final Collector<Object, ?, ReferenceBigArrayBigList<Object>> TO_LIST_COLLECTOR =
	 Collector.of(
	  ReferenceBigArrayBigList::new,
	  ReferenceBigArrayBigList::add,
	  ReferenceBigArrayBigList::combine);
	 ...
	 *   This can be quite wasteful, as worker threads other then the first don't usually handle the contents of the full stream.
	 ...
	 public static <K> Collector<K, ?, ReferenceBigArrayBigList <K> > toBigListWithExpectedSize(long expectedSize) {
	  return Collector.of(
	 () -> new ReferenceBigArrayBigList <K>(expectedSize),
	 ReferenceBigArrayBigList::add,
	 ReferenceBigArrayBigList::combine);
	}
	...
	private class SubList extends AbstractReferenceBigList.ReferenceRandomAccessSubList <K> {
	 ...
	 private final class SubListIterator extends ObjectBigListIterators.AbstractIndexBasedBigListIterator <K> {
	  ...
	  @Override
	  protected final K get(long i) { return BigArrays.get(a, from + i); }
	  @Override
	  protected final void add(long i, K k) { SubList.this.add(i, k); }
	  @Override
	  protected final void set(long i, K k) { SubList.this.set(i, k); }
	  @Override
	  protected final void remove(long i) { SubList.this.remove(i); }
	  @Override
	  protected final long getMaxPos() { return to - from; }
	  @Override
	  public K next() { if (! hasNext()) throw new NoSuchElementException(); return BigArrays.get(a, from + (lastReturned = pos++)); }
	  @Override
	  public K previous() { if (! hasPrevious()) throw new NoSuchElementException(); return BigArrays.get(a, from + (lastReturned = --pos)); }
	  @Override
	 ...
	 private final class SubListSpliterator extends ObjectBigSpliterators.LateBindingSizeIndexBasedSpliterator <K> {
	  ...
	  protected final long getMaxPosFromBackingStore() { return to; }
	   @Override
	  protected final K get(long i) { return BigArrays.get(a, i); }
	  @Override
	...
	public ObjectBigListIterator <K> listIterator(final long index) {
	 ...
	 return new ObjectBigListIterator <K>() {
	   ...
	   @Override
	   public K next() { if (! hasNext()) throw new NoSuchElementException(); return BigArrays.get(a, last = pos++); }
	   @Override
	   public K previous() { if (! hasPrevious()) throw new NoSuchElementException(); return BigArrays.get(a, last = --pos); }
	   @Override
	...
	private final class Spliterator implements ObjectSpliterator <K> {
	 ...
	 @Override
	 public int characteristics() { return ObjectSpliterators.LIST_SPLITERATOR_CHARACTERISTICS; }
	 @Override
