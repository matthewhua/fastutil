...
public class ShortBigArrayBigList extends AbstractShortBigList implements RandomAccess, Cloneable, java.io.Serializable {
	...
	private class SubList extends AbstractShortBigList.ShortRandomAccessSubList {
	 ...
	 private final class SubListIterator extends ShortBigListIterators.AbstractIndexBasedBigListIterator {
	  ...
	  @Override
	  protected final short get(long i) { return BigArrays.get(a, from + i); }
	  @Override
	  protected final void add(long i, short k) { SubList.this.add(i, k); }
	  @Override
	  protected final void set(long i, short k) { SubList.this.set(i, k); }
	  @Override
	  protected final void remove(long i) { SubList.this.removeShort(i); }
	  @Override
	  protected final long getMaxPos() { return to - from; }
	  @Override
	  public short nextShort() { if (! hasNext()) throw new NoSuchElementException(); return BigArrays.get(a, from + (lastReturned = pos++)); }
	  @Override
	  public short previousShort() { if (! hasPrevious()) throw new NoSuchElementException(); return BigArrays.get(a, from + (lastReturned = --pos)); }
	  @Override
	 ...
	 private final class SubListSpliterator extends ShortBigSpliterators.LateBindingSizeIndexBasedSpliterator {
	  ...
	  protected final long getMaxPosFromBackingStore() { return to; }
	   @Override
	  protected final short get(long i) { return BigArrays.get(a, i); }
	  @Override
	...
	public ShortBigListIterator listIterator(final long index) {
	 ...
	 return new ShortBigListIterator () {
	   ...
	   @Override
	   public short nextShort() { if (! hasNext()) throw new NoSuchElementException(); return BigArrays.get(a, last = pos++); }
	   @Override
	   public short previousShort() { if (! hasPrevious()) throw new NoSuchElementException(); return BigArrays.get(a, last = --pos); }
	   @Override
	...
	private final class Spliterator implements ShortSpliterator {
	 ...
	 @Override
	 public int characteristics() { return ShortSpliterators.LIST_SPLITERATOR_CHARACTERISTICS; }
	 @Override
