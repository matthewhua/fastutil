...
public interface ShortCollection extends Collection<Short>, ShortIterable {
	...
	default ShortSpliterator spliterator() {
	 return ShortSpliterators.asSpliterator(
	   iterator(), sizeOf(this), ShortSpliterators.COLLECTION_SPLITERATOR_CHARACTERISTICS);
	}
	...
	default boolean removeIf(final java.util.function.Predicate<? super Short> filter) {
	 return removeIf(
	  filter instanceof ShortPredicate ?
	   ((ShortPredicate ) filter) :
	   (ShortPredicate ) key -> filter.test(Short.valueOf(it.unimi.dsi.fastutil.SafeMath.safeIntToShort(key))));
	}
	...
	@Deprecated
	@Override
	default java.util.stream.Stream<Short> stream() { return Collection.super.stream(); }
	...
	@Deprecated
	@Override
	default java.util.stream.Stream<Short> parallelStream() { return Collection.super.parallelStream(); }
