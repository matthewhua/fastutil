...
public abstract class AbstractIntBigList extends AbstractIntCollection implements IntBigList , IntStack {
	...
	public IntBigListIterator listIterator(final long index) {
	 ...
	 return new IntBigListIterators.AbstractIndexBasedBigListIterator (0, index) {
	  @Override
	  protected final int get(long i) { return AbstractIntBigList.this.getInt(i); }
	  @Override
	  protected final void add(long i, int k) { AbstractIntBigList.this.add(i, k); }
	  @Override
	  protected final void set(long i, int k) { AbstractIntBigList.this.set(i, k); }
	  @Override
	  protected final void remove(long i) { AbstractIntBigList.this.removeInt(i); }
	  @Override
	  protected final long getMaxPos() { return AbstractIntBigList.this.size64(); }
	 };
	}
	static final class IndexBasedSpliterator extends IntBigSpliterators.LateBindingSizeIndexBasedSpliterator {
	 ...
	 @Override
	 protected final int get(long i) { return l.getInt(i); }
	 @Override
	...
	public static class IntSubList extends AbstractIntBigList implements java.io.Serializable {
	 ...
	 private final class RandomAccessIter extends IntBigListIterators.AbstractIndexBasedBigListIterator {
	  ...
	  @Override
	  protected final int get(long i) { return l.getInt(from + i); }
	  // Remember, these are calling SUBLIST's methods, meaning 0 is the start of the sublist for these.
	  @Override
	  protected final void add(long i, int k) { IntSubList.this.add(i, k); }
	  @Override
	  protected final void set(long i, int k) { IntSubList.this.set(i, k); }
	  @Override
	  protected final void remove(long i) { IntSubList.this.removeInt(i); }
	  @Override
	 ...
	 private class ParentWrappingIter implements IntBigListIterator {
	  ...
	  @Override
	  public int nextInt() { if (! hasNext()) throw new NoSuchElementException(); return parent.nextInt(); }
	  @Override
	  public int previousInt() { if (! hasPrevious()) throw new NoSuchElementException(); return parent.previousInt(); }
	  @Override
	  public void add(int k) { parent.add(k); }
	  @Override
	  public void set(int k) { parent.set(k); }
	  @Override
