...
public class LongArrayList extends AbstractLongList implements RandomAccess, Cloneable, java.io.Serializable {
	...
	public static LongArrayList toListWithExpectedSize(java.util.stream.LongStream stream, int expectedSize) {
	 ...
	 return stream.collect(
	  new LongCollections.SizeDecreasingSupplier<
	    LongArrayList >(
	   expectedSize, (int size) ->
	    size <= DEFAULT_INITIAL_CAPACITY ? new LongArrayList () : new LongArrayList (size)),
	  LongArrayList::add,
	  LongArrayList::addAll);
	}
	...
	private class SubList extends AbstractLongList.LongRandomAccessSubList {
	 ...
	 private final class SubListIterator extends LongIterators.AbstractIndexBasedListIterator {
	  ...
	  @Override
	  protected final long get(int i) { return a[from + i]; }
	  @Override
	  protected final void add(int i, long k) { SubList.this.add(i, k); }
	  @Override
	  protected final void set(int i, long k) { SubList.this.set(i, k); }
	  @Override
	  protected final void remove(int i) { SubList.this.removeLong(i); }
	  @Override
	  protected final int getMaxPos() { return to - from; }
	  @Override
	  public long nextLong() { if (! hasNext()) throw new NoSuchElementException(); return a[from + (lastReturned = pos++)]; }
	  @Override
	  public long previousLong() { if (! hasPrevious()) throw new NoSuchElementException(); return a[from + (lastReturned = --pos)]; }
	  @Override
