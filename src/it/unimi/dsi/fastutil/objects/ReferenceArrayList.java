...
public class ReferenceArrayList <K> extends AbstractReferenceList <K> implements RandomAccess, Cloneable, java.io.Serializable {
	...
	public static <K> Collector<K, ?, ReferenceArrayList <K> > toList() {
	 ...
	 public static <K> Collector<K, ?, ReferenceArrayList <K> > toListWithExpectedSize(int expectedSize) {
	  ...
	  expectedSize, (int size) ->
	   size <= DEFAULT_INITIAL_CAPACITY ? new ReferenceArrayList <K>() : new ReferenceArrayList <K>(size)),
	 ReferenceArrayList::add,
	 ReferenceArrayList::combine);
	}
	...
	private class SubList extends AbstractReferenceList.ReferenceRandomAccessSubList <K> {
	 ...
	 private final class SubListIterator extends ObjectIterators.AbstractIndexBasedListIterator <K> {
	  ...
	  @Override
	  protected final K get(int i) { return a[from + i]; }
	  @Override
	  protected final void add(int i, K k) { SubList.this.add(i, k); }
	  @Override
	  protected final void set(int i, K k) { SubList.this.set(i, k); }
	  @Override
	  protected final void remove(int i) { SubList.this.remove(i); }
	  @Override
	  protected final int getMaxPos() { return to - from; }
	  @Override
	  public K next() { if (! hasNext()) throw new NoSuchElementException(); return a[from + (lastReturned = pos++)]; }
	  @Override
	  public K previous() { if (! hasPrevious()) throw new NoSuchElementException(); return a[from + (lastReturned = --pos)]; }
	  @Override
