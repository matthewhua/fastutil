...
public class IntBigArrayBigList extends AbstractIntBigList implements RandomAccess, Cloneable, java.io.Serializable {
	 ...
	 */
	 public static IntBigArrayBigList toBigList(java.util.stream.IntStream stream) {
	  return stream.collect(
	   IntBigArrayBigList::new,
	   IntBigArrayBigList::add,
	   IntBigArrayBigList::addAll);
	 }
	/** Collects the result of a primitive {@code Stream} into a new BigArrayBigList.
	 *
	 * <p>This method performs a terminal operation on the given {@code Stream}
	...
	public static IntBigArrayBigList toBigListWithExpectedSize(java.util.stream.IntStream stream, long expectedSize) {
	 return stream.collect(
	  () -> new IntBigArrayBigList (expectedSize),
	  IntBigArrayBigList::add,
	  IntBigArrayBigList::addAll);
	}
	...
	private class SubList extends AbstractIntBigList.IntRandomAccessSubList {
	 ...
	 private final class SubListIterator extends IntBigListIterators.AbstractIndexBasedBigListIterator {
	  ...
	  @Override
	  protected final int get(long i) { return BigArrays.get(a, from + i); }
	  @Override
	  protected final void add(long i, int k) { SubList.this.add(i, k); }
	  @Override
	  protected final void set(long i, int k) { SubList.this.set(i, k); }
	  @Override
	  protected final void remove(long i) { SubList.this.removeInt(i); }
	  @Override
	  protected final long getMaxPos() { return to - from; }
	  @Override
	  public int nextInt() { if (! hasNext()) throw new NoSuchElementException(); return BigArrays.get(a, from + (lastReturned = pos++)); }
	  @Override
	  public int previousInt() { if (! hasPrevious()) throw new NoSuchElementException(); return BigArrays.get(a, from + (lastReturned = --pos)); }
	  @Override
	 ...
	 private final class SubListSpliterator extends IntBigSpliterators.LateBindingSizeIndexBasedSpliterator {
	  ...
	  protected final long getMaxPosFromBackingStore() { return to; }
	   @Override
	  protected final int get(long i) { return BigArrays.get(a, i); }
	  @Override
