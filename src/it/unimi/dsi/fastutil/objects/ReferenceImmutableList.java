...
public class ReferenceImmutableList <K> extends ReferenceLists.ImmutableListBase <K> implements ReferenceList <K>, RandomAccess, Cloneable, java.io.Serializable {
	...
	private static final Collector<Object, ?, ReferenceImmutableList<Object>> TO_LIST_COLLECTOR =
	 Collector.of(
	...
	public static <K> Collector<K, ?, ReferenceImmutableList <K> > toList() {
	 ...
	 public static <K> Collector<K, ?, ReferenceImmutableList <K> > toListWithExpectedSize(int expectedSize) {
	  ...
	  return Collector.<K, ReferenceArrayList <K>, ReferenceImmutableList <K> >of(
	  new ReferenceCollections.SizeDecreasingSupplier<
	    K,
	    ReferenceArrayList <K> >(
	   expectedSize, (int size) ->
	    size <= ReferenceArrayList.DEFAULT_INITIAL_CAPACITY ? new ReferenceArrayList <K>() : new ReferenceArrayList <K>(size)),
	  ReferenceArrayList::add,
	  ReferenceArrayList::combine,
	  ReferenceImmutableList::convertTrustedToImmutableList);
	}
	...
	public ObjectListIterator <K> listIterator(final int index) {
	 ...
	 return new ObjectListIterator <K>() {
	   ...
	   @Override
	   public K next() { if (! hasNext()) throw new NoSuchElementException(); return a[pos++]; }
	   @Override
	   public K previous() { if (! hasPrevious()) throw new NoSuchElementException(); return a[--pos]; }
	   @Override
	 ...
	 @Override
	 public int characteristics() { return ObjectSpliterators.LIST_SPLITERATOR_CHARACTERISTICS | java.util.Spliterator.IMMUTABLE; }
	 @Override
	...
	private final static class ImmutableSubList <K> extends ReferenceLists.ImmutableListBase <K> implements java.util.RandomAccess, java.io.Serializable {
	 ...
	 public ObjectListIterator <K> listIterator(final int index) {
	  ...
	  return new ObjectListIterator <K>() {
	    ...
	    @Override
	    public K next() { if (! hasNext()) throw new NoSuchElementException(); return a[pos++ + from]; }
	    @Override
	    public K previous() { if (! hasPrevious()) throw new NoSuchElementException(); return a[--pos + from]; }
	    @Override
	    public int nextIndex() { return pos; }
	    @Override
	    public int previousIndex() { return pos - 1; }
	 ...
	 private final class SubListSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator <K> {
	  ...
	  // Remember, the indexes we are getting is the real array's index, not our sublist relative index.
	   @Override
	  protected final K get(int i) { return a[i]; }
	  @Override
