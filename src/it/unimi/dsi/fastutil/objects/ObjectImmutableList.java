...
public class ObjectImmutableList <K> extends ObjectLists.ImmutableListBase <K> implements ObjectList <K>, RandomAccess, Cloneable, java.io.Serializable {
	...
	public static <K> Collector<K, ?, ObjectImmutableList <K> > toList() {
	 ...
	 public static <K> Collector<K, ?, ObjectImmutableList <K> > toListWithExpectedSize(int expectedSize) {
	  ...
	  return Collector.<K, ObjectArrayList <K>, ObjectImmutableList <K> >of(
	  new ObjectCollections.SizeDecreasingSupplier<
	    K,
	    ObjectArrayList <K> >(
	   expectedSize, (int size) ->
	    size <= ObjectArrayList.DEFAULT_INITIAL_CAPACITY ? new ObjectArrayList <K>() : new ObjectArrayList <K>(size)),
	  ObjectArrayList::add,
	  ObjectArrayList::combine,
	  ObjectImmutableList::convertTrustedToImmutableList);
	}
