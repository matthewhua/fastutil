...
public class LongBigArrayBigList extends AbstractLongBigList implements RandomAccess, Cloneable, java.io.Serializable {
	 ...
	 */
	 public static LongBigArrayBigList toBigList(java.util.stream.LongStream stream) {
	  return stream.collect(
	   LongBigArrayBigList::new,
	   LongBigArrayBigList::add,
	   LongBigArrayBigList::addAll);
	 }
	/** Collects the result of a primitive {@code Stream} into a new BigArrayBigList.
	 *
	 * <p>This method performs a terminal operation on the given {@code Stream}
	...
	public static LongBigArrayBigList toBigListWithExpectedSize(java.util.stream.LongStream stream, long expectedSize) {
	 return stream.collect(
	  () -> new LongBigArrayBigList (expectedSize),
	  LongBigArrayBigList::add,
	  LongBigArrayBigList::addAll);
	}
	...
	private class SubList extends AbstractLongBigList.LongRandomAccessSubList {
	 ...
	 private final class SubListIterator extends LongBigListIterators.AbstractIndexBasedBigListIterator {
	  ...
	  @Override
	  protected final long get(long i) { return BigArrays.get(a, from + i); }
	  @Override
	  protected final void add(long i, long k) { SubList.this.add(i, k); }
	  @Override
	  protected final void set(long i, long k) { SubList.this.set(i, k); }
	  @Override
	  protected final void remove(long i) { SubList.this.removeLong(i); }
	  @Override
	  protected final long getMaxPos() { return to - from; }
	  @Override
	  public long nextLong() { if (! hasNext()) throw new NoSuchElementException(); return BigArrays.get(a, from + (lastReturned = pos++)); }
	  @Override
	  public long previousLong() { if (! hasPrevious()) throw new NoSuchElementException(); return BigArrays.get(a, from + (lastReturned = --pos)); }
	  @Override
	 ...
	 private final class SubListSpliterator extends LongBigSpliterators.LateBindingSizeIndexBasedSpliterator {
	  ...
	  protected final long getMaxPosFromBackingStore() { return to; }
	   @Override
	  protected final long get(long i) { return BigArrays.get(a, i); }
	  @Override
	...
	public LongBigListIterator listIterator(final long index) {
	 ...
	 return new LongBigListIterator () {
	   ...
	   @Override
	   public long nextLong() { if (! hasNext()) throw new NoSuchElementException(); return BigArrays.get(a, last = pos++); }
	   @Override
	   public long previousLong() { if (! hasPrevious()) throw new NoSuchElementException(); return BigArrays.get(a, last = --pos); }
	   @Override
