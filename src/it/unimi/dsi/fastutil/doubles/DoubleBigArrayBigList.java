...
public class DoubleBigArrayBigList extends AbstractDoubleBigList implements RandomAccess, Cloneable, java.io.Serializable {
	 ...
	 */
	 public static DoubleBigArrayBigList toBigList(java.util.stream.DoubleStream stream) {
	  return stream.collect(
	   DoubleBigArrayBigList::new,
	   DoubleBigArrayBigList::add,
	   DoubleBigArrayBigList::addAll);
	 }
	/** Collects the result of a primitive {@code Stream} into a new BigArrayBigList.
	 *
	 * <p>This method performs a terminal operation on the given {@code Stream}
	...
	public static DoubleBigArrayBigList toBigListWithExpectedSize(java.util.stream.DoubleStream stream, long expectedSize) {
	 return stream.collect(
	  () -> new DoubleBigArrayBigList (expectedSize),
	  DoubleBigArrayBigList::add,
	  DoubleBigArrayBigList::addAll);
	}
	...
	private class SubList extends AbstractDoubleBigList.DoubleRandomAccessSubList {
	 ...
	 private final class SubListIterator extends DoubleBigListIterators.AbstractIndexBasedBigListIterator {
	  ...
	  @Override
	  protected final double get(long i) { return BigArrays.get(a, from + i); }
	  @Override
	  protected final void add(long i, double k) { SubList.this.add(i, k); }
	  @Override
	  protected final void set(long i, double k) { SubList.this.set(i, k); }
	  @Override
	  protected final void remove(long i) { SubList.this.removeDouble(i); }
	  @Override
	  protected final long getMaxPos() { return to - from; }
	  @Override
	  public double nextDouble() { if (! hasNext()) throw new NoSuchElementException(); return BigArrays.get(a, from + (lastReturned = pos++)); }
	  @Override
	  public double previousDouble() { if (! hasPrevious()) throw new NoSuchElementException(); return BigArrays.get(a, from + (lastReturned = --pos)); }
	  @Override
	 ...
	 private final class SubListSpliterator extends DoubleBigSpliterators.LateBindingSizeIndexBasedSpliterator {
	  ...
	  protected final long getMaxPosFromBackingStore() { return to; }
	   @Override
	  protected final double get(long i) { return BigArrays.get(a, i); }
	  @Override
	...
	public DoubleBigListIterator listIterator(final long index) {
	 ...
	 return new DoubleBigListIterator () {
	   ...
	   @Override
	   public double nextDouble() { if (! hasNext()) throw new NoSuchElementException(); return BigArrays.get(a, last = pos++); }
	   @Override
	   public double previousDouble() { if (! hasPrevious()) throw new NoSuchElementException(); return BigArrays.get(a, last = --pos); }
	   @Override
